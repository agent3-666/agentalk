...
const DEFAULT_AGENTS = [
  ...
  {
    // ZCode — z.ai's native coding agent CLI, running the GLM Coding Plan
    // (subscription billing pool). The CLI ships *inside* the ZCode desktop
    // app bundle at Contents/Resources/glm/zcode.cjs and needs a Node runtime
    // with the built-in node:sqlite module (Node 22.5+), which the system
    // default node may lack. The recommended setup is a small wrapper on PATH:
    //
    //   #!/bin/sh
    //   ZCODE_CJS="/Applications/ZCode.app/Contents/Resources/glm/zcode.cjs"
    //   exec "${ZCODE_NODE:-$(command -v node22 || command -v node)}" "$ZCODE_CJS" "$@"
    //
    // The active model is set once in ~/.zcode/cli/config.json, e.g.:
    //   { "provider": { "zai-coding-plan": { "kind": "anthropic",
    //       "options": { "baseURL": "https://api.z.ai/api/anthropic",
    //                    "apiKey": "<z.ai coding-plan key>" } } },
    //     "model": { "main": "zai-coding-plan/glm-5.2" } }
    // (model.main is a "providerId/modelId" string, not an object.)
    key: "zcode",
    name: "ZCode",
    cmd: "zcode",
    // `--verbose` is load-bearing: zcode's headless `--prompt` mode is BATCH,
    // not streaming — it works silently and only prints the final answer to
    // stdout at the very end (verified: stdout stays 0 bytes until done).
    // Without ongoing output the supervisor's inactivity watchdog mistakes a
    // healthy-but-quiet zcode for a hang and kills it mid-task. `--verbose`
    // emits step diagnostics to STDERR (not stdout, so it doesn't pollute the
    // captured answer) which keeps the watchdog fed. Paired with the generous
    // inactivity_timeout_sec below and parseQuotaSignal's "recovered → ok"
    // rule (a non-empty answer overrides noisy stderr).
    args: ["--prompt", "{prompt}", "--mode", "yolo", "--no-color", "--verbose"],
    color: "#6C5CE7",
    output: "text",
    enabled: true,
    model: null,
    // zcode is non-streaming on stdout and can think/act silently for minutes
    // on a real task; treat stdout-silence like opencode, not as a hang. 900s
    // covers realistic long tasks; 0 would disable it and rely only on total
    // --timeout.
    inactivity_timeout_sec: 900,
    note: "ZCode CLI (z.ai GLM Coding Plan, native harness). Model is configured in ~/.zcode/cli/config.json. Needs a `zcode` wrapper on PATH backed by Node 22.5+ (the CLI uses node:sqlite). See comment above for setup. Batch/non-streaming: runs with --verbose so the inactivity watchdog sees progress.",
  },
  {
    key: "opencode",
    name: "OpenCode",
    ...
    output: "ndjson",
    detect_name: true,
    // Disabled by default in favor of `zcode` (z.ai's native harness on the
    // same GLM Coding Plan subscription). Re-enable with `/agents enable
    // opencode` if you prefer the OpenCode harness or use it with non-GLM
    // backends.
    enabled: false,
    model: null,
    // Per-agent override of supervisor's default inactivity watchdog.
    // opencode batches file edits internally (esp. with GLM backend) and
    // can be silent for 10+ minutes mid-task without being hung. 900s
    // (15 min) covers realistic long-edit cases; 0 disables, relying
    // only on total --timeout.
    inactivity_timeout_sec: 900,
    note: "OpenCode CLI (supports GLM, GPT, etc.). Disabled by default in favor of zcode.",
  },
  {
    key: "claude",
    name: "Claude",
    ...
    model: null,
    model_flag: "--model",
    // shares_auth_with_host: hosts where this CLI uses the same OAuth/quota
    // as the host process. agentalk auto-skips matching agents at startup.
    // Currently only "claude-code" is recognized; future entries (e.g.
    // "cursor", "windsurf") would extend the same mechanism.
    shares_auth_with_host: ["claude-code"],
    note: "Anthropic Claude Code. Disabled by default: when agentalk is invoked from inside Claude Code, the `claude` CLI shares the host's OAuth/quota and double-bills the user. Enable with `/agents enable claude` if you run agentalk from a plain terminal and want Claude in panels.",
  },
...
export function loadConfig() {
  ...
  try {
    ...
    for (const agent of saved.agents) {
      ...
      // Migrate: backfill per-agent inactivity watchdog from defaults when
      // missing. Added after the hardcoded 5-min watchdog was killing
      // valid long-running opencode/GLM tasks. Explicit 0 (disabled) is
      // left alone — only null/undefined gets filled.
      if (def && def.inactivity_timeout_sec != null && agent.inactivity_timeout_sec == null) {
        agent.inactivity_timeout_sec = def.inactivity_timeout_sec;
        updated = true;
      }
...
// forceIncludeKeys: agents in this set bypass the `enabled=false` gate
// (still need to be installed). Used by --include-claude as a single escape
// hatch: user wants claude even though config has it disabled.
export function getActiveAgents({ forceIncludeKeys = [] } = {}) {
  const config = loadConfig();
  const agents = {};
  const defaultsByKey = Object.fromEntries(DEFAULT_AGENTS.map(d => [d.key, d]));
  const forceSet = new Set(forceIncludeKeys);

  // Detect Claude's current model once for all claude-keyed agents
  let claudeDetected = null;

  for (const def of config.agents) {
    if (!def.enabled && !forceSet.has(def.key)) continue;
    ...
    agents[def.key] = {
      ...
      // shares_auth_with_host falls back to DEFAULT_AGENTS for built-in keys —
      // existing user configs predating this field won't have it. The CLI's
      // auth-sharing relationship is a fact about the tool, not user preference.
      shares_auth_with_host: Array.isArray(def.shares_auth_with_host)
        ? [...def.shares_auth_with_host]
        : (defaultsByKey[def.key]?.shares_auth_with_host || []),
      // inactivity_timeout_sec: per-agent override of supervisor's stream-silence
      // watchdog. Falls back to DEFAULT_AGENTS for the same migration reason as above.
      inactivity_timeout_sec: typeof def.inactivity_timeout_sec === "number"
        ? def.inactivity_timeout_sec
        : (typeof defaultsByKey[def.key]?.inactivity_timeout_sec === "number"
            ? defaultsByKey[def.key].inactivity_timeout_sec
            : null),
    };
  }
...
export function addAgent(def) {
  ...
  // Uniform policy: "added ≠ enabled". A new agent lands DISABLED unless the
  // caller explicitly passes enabled:true. This keeps user-added agents
  // (often pay-per-call API models) consistent with the built-in pay-per-call
  // defaults, which also ship disabled — adding a model never silently starts
  // billing. Enable explicitly via enableAgent() / `/agents enable <key>`.
  const enabled = def.enabled === true;
  const entry = { ...def, enabled };
  ...
  if (enabledIndices.length > 0) {
    ...
    config.agents.splice(lastEnabledIdx, 0, entry);
  } else {
    config.agents.push(entry);
  }
  saveConfig(config);
  return { ok: true, msg: `已添加 ${def.name}`, enabled };
}
