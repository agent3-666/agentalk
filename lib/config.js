...
const DEFAULT_AGENTS = [
  {
    ...
    name: "Codex",
    cmd: "codex",
    args: ["exec", "--skip-git-repo-check", "-c", "sandbox_permissions=[\"disk-full-read-access\"]", "{prompt}"],
    color: "#10B981",
    output: "text",
    enabled: true,
    model: null,
...
export function loadConfig() {
  ...
  try {
    ...
    for (const agent of saved.agents) {
      ...
      // Migrate: add disk-full-read-access sandbox permission so codex can
      // write files during delegation. Old args lacked the -c flag entirely.
      if (agent.key === "codex" && !agent.args.includes("disk-full-read-access")) {
...
export function getActiveAgents() {
  ...
  for (const def of config.agents) {
    ...
    // For Claude with no explicit model set, try to detect from recent session files.
