...
export function createRepl({ prompt, commands, onLine, onSigint }) {
  ...
  // Piped stdin is not an interactive REPL. Treating each line as a separate
  // command can fan one prompt file into dozens of agent calls. Headless modes
  // should use explicit --stdin/--file flags; this fallback only preserves a
  // one-line pipe for simple script compatibility.
  if (!process.stdin.isTTY) {
    ...
    process.stdin.on("end", async () => {
      ...
      if (nonEmptyLines.length > 1) {
        console.error(chalk.red("Refusing to run multi-line piped input as an interactive agentalk session."));
        console.error(chalk.dim("Use a headless mode with --stdin/--file, for example:"));
        console.error(chalk.dim("  agentalk --deepen --stdin < prompt.txt"));
        console.error(chalk.dim("For implementation sub-tasks, use agentalk-delegate instead:"));
        console.error(chalk.dim("  agentalk-delegate gemini \"<task>\" --files \"path1,path2\""));
        process.exit(2);
      }
  ...
  let pausedTyped = false; // whether user started typing during current pause

  // History state
  let history = [];
  let historyIdx = -1;
  let historySaved = ""; // saved current buf when user starts navigating history
  ...
  function renderPicker() {
    ...
    const lines = [chalk.dim("  ↑↓ navigate  Space=toggle  Enter=confirm  Esc=cancel\n")];
    for (let i = 0; i < pickerItems.length; i++) {
      const item = pickerItems[i];
      const checked = pickerSelected.has(item.key) ? chalk.green("✓") : " ";
      const arrow = i === pickerCursor ? chalk.cyan("▶") : " ";
      const key = i === pickerCursor ? chalk.bold.white(item.key.padEnd(12)) : chalk.cyan(item.key.padEnd(12));
      const name = i === pickerCursor ? chalk.white(item.name) : chalk.dim(item.name);
      lines.push(`  ${arrow} [${checked}] ${key} ${name}\n`);
    }
    process.stdout.write(lines.join(""));
    prevCursorRow = lines.length; // each line ends with \n → cursor is that many rows below start
  }
  ...
  function pause() {
    ...
    process.stdout.write(`\r\x1b[J`);
    prevCursorRow = 0;
    // During pause we can't show a live editable input line — agent output
    // is streaming and would collide with any re-rendered prompt. Keystrokes
    // are captured silently and confirmed by a one-time ack when the user
    // first types. Enter submits, Ctrl+C stops.
  }
  ...
  function handleKey(name) {
    // ── Picker mode: intercept all navigation ──
    if (pickerMode) {
      if (name === "up") {
        pickerCursor = Math.max(0, pickerCursor - 1);
        renderPicker();
      } else if (name === "down") {
        pickerCursor = Math.min(pickerItems.length - 1, pickerCursor + 1);
        renderPicker();
      } else if (name === "return") {
        pickerMode = false;
        ...
        process.stdout.write("\n");
        prevCursorRow = 0;
        resolve([...pickerSelected]);
      } else if (name === "escape" || name === "ctrl-c") {
        pickerMode = false;
        const resolve = pickerResolve;
        pickerResolve = null;
        process.stdout.write("\n");
    ...
    // Keystrokes are buffered silently (no live echo, because agent streams
    // would overwrite a rendered prompt). Enter submits the buffer.
    if (paused) {
  ...
  function handleChar(ch) {
    // Picker mode: space toggles selection
    if (pickerMode) {
      if (ch === " ") {
        const key = pickerItems[pickerCursor]?.key;
        if (key) {
    ...
    if (paused) {
      // First keystroke during pause → print a one-time ack so the user
      // knows their typing is being captured (even though it's not echoed).
      if (!pausedTyped) {
        pausedTyped = true;
        process.stdout.write(`\n${chalk.dim(t("input.paused_typing_ack"))}\n`);
      }
  ...
  // ── Interactive picker ──
  // items: [{key, name}], preSelected: Set of keys to check by default
  // Returns Promise<string[]> of selected keys, or null if cancelled.
  function showPicker(items, preSelected = new Set()) {
    return new Promise((resolve) => {
      pickerItems = items;
      pickerCursor = 0;
      pickerSelected = new Set(preSelected);
  ...
  return { showPrompt, pause, showPicker };
}
