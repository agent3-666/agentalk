...
export function createRepl({ prompt, commands, onLine, onSigint }) {
  ...
  function render() {
    ...
    if (pasteContent) {
      ...
      const indicatorText = ` [Pasted: ${lines} lines, ${chars} chars] `;
  ...
  function showPrompt() {
    ...
    if (scrollMode) {
      const rows = process.stdout.rows || 24;
      // Clear fixed input line
      process.stdout.write(`\x1b7\x1b[${rows};1H\x1b[2K\x1b8`);
      // DECSTBM moves cursor to (1,1) in most terminals — save/restore around it
      process.stdout.write(`\x1b7\x1b[1;${rows}r\x1b8`);
      scrollMode = false;
    }
    render();
  }
  ...
  function pause() {
    ...
    if (process.stdout.isTTY) {
      ...
      if (rows > 3) {
        ...
        // Draw input prompt on the fixed bottom line
        updateFixedInput();
      }
    }
  }

  // Handle terminal resize during scroll mode
  if (process.stdout.isTTY) {
    process.stdout.on("resize", () => {
      if (scrollMode) {
        const rows = process.stdout.rows || 24;
        // Save/restore around DECSTBM
        process.stdout.write(`\x1b7\x1b[1;${rows - 1}r\x1b8`);
        updateFixedInput();
      }
    });
  ...
  function handleKey(name) {
    ...
    if (name === "ctrl-c") {
      if (paused) {
        // Discussion in progress → send stop signal
        clearSuggestions();
        pasteContent = "";
        onSigint?.();
        return;
      ...
      lastCtrlC = now;
      process.stdout.write(`\n${chalk.dim("再按一次 Ctrl+C 退出，或继续输入")}\n`);
      render();
      return;
    }
