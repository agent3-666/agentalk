...
function streamOpencode(proc, agent, silent, resolve) {
  ...
  proc.on("close", (code) => {
    ...
    if (!silent) {
      ...
      console.log(`${agent.color("╰")} ${chalk.dim(`${elapsed}s`)}`);
    }
    resolve({ agent: "opencode", response: textBuffer.trim(), error: stderr.trim(), elapsed });
  });
...
export function runAgent(agentKey, prompt, { silent = false } = {}) {
  ...
  return new Promise((resolve) => {
    ...
    proc.on("close", (code) => {
      ...
      resolve({ agent: agentKey, response: stdout.trim(), error: stderr.trim(), elapsed });
    });
