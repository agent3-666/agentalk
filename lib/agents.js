...
import { getActiveAgents, getGlobalTimeout, isCallerClaudeCode } from "./config.js";
import { t } from "./i18n.js";
import { tryConsume, isEnabled as budgetEnabled } from "./budget.js";

// ─── Load agents from config at startup ─────────────────────────────
// When running inside Claude Code (CLAUDECODE=1 or --from-claude), skip any
// agent that invokes the `claude` CLI — it shares the same OAuth quota as the
// host and would double-bill the user. Pass --include-claude to override.
const _callerIsClaudeCode = isCallerClaudeCode();
const _includeClaude = process.argv.includes("--include-claude");

// Hosts where agentalk auto-detects whether it's running. Right now only
// "claude-code" is detected; this is the same string used in the agent's
// `shares_auth_with_host` field so an agent declares which hosts share its auth.
function detectHosts() {
  const hosts = [];
  if (_callerIsClaudeCode) hosts.push("claude-code");
  return hosts;
}

function loadFilteredAgents() {
  // --include-claude is a single escape hatch: bypasses both the CLAUDECODE
  // auto-filter (below) AND the `enabled=false` config gate (in getActiveAgents).
  const defs = getActiveAgents({
    forceIncludeKeys: _includeClaude ? ["claude"] : [],
  });
  const hosts = detectHosts();
  if (hosts.length > 0 && !_includeClaude) {
    // An agent is filtered if it declares it shares auth with any host we're inside of.
    const skipped = Object.keys(defs).filter(k => {
      const shared = defs[k].shares_auth_with_host || [];
      return shared.some(h => hosts.includes(h));
    });
    for (const k of skipped) delete defs[k];
    if (skipped.length > 0) {
      const hostLabel = hosts.map(h => h === "claude-code" ? "Claude Code" : h).join("/");
      process.stderr.write(
        chalk.yellow(`⚠ agentalk: [${skipped.join(", ")}] skipped — host is ${hostLabel} (shares quota). Pass --include-claude to override.\n`)
      );
    }
  }
  return defs;
...
export const AGENTS = buildAgents(loadFilteredAgents());
...
function buildAgents(defs) {
  ...
  for (const [key, def] of Object.entries(defs)) {
    ...
    agents[key] = {
      ...
      // Per-agent inactivity-watchdog override (read by supervisor.delegateCore).
      // null = use supervisor's DEFAULT_INACTIVITY_SEC; 0 = disable watchdog.
      inactivity_timeout_sec: typeof def.inactivity_timeout_sec === "number"
        ? def.inactivity_timeout_sec
        : null,
...
function streamOpencode(proc, agent, silent, resolve, spinner, settledRef, onStdout, onStderr) {
  ...
  let rawBuffer = "";
  let stderr = "";
  let realTokens = null; // accumulated from step_finish events
    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (obj.type === "step_finish" && obj.part?.tokens) {
          ...
          if (!realTokens) realTokens = { prompt: 0, completion: 0, total: 0, estimated: false };
          realTokens.prompt += t.input || 0;
          realTokens.completion += t.output || 0;
          realTokens.total += t.total || 0;
        }
      } catch {}
    }
  });
  ...
  proc.on("close", (code) => {
    ...
    const tokens = realTokens || { total: Math.ceil(textBuffer.length / 3), estimated: true };
    if (!silent) {
...
// ─── Rate-limit storm fast-fail ──────────────────────────────────────
// Batch agents on subscription providers (notably zcode → z.ai's GLM Coding
// Plan) can hit the provider's rate limit on their very FIRST call and then
// spin inside their own internal retry loop: they emit only rate-limit errors
// to stderr and never a byte of stdout. That stderr traffic keeps the
// inactivity watchdog alive, so the child otherwise burns the ENTIRE hard
// timeout (observed 2026-07-01: 23 min, 82 consecutive z.ai [1302]s, zero
// output) and returns nothing. We detect the storm — sustained rate-limit
// stderr with no stdout ever produced — and kill fast with a clean quota
// error, so the caller reroutes (e.g. to codex) in ~60s instead of ~23 min.
// Guarded by `!stdoutEverSeen`: once any real output arrives the agent is
// making progress and we never fast-fail, so a couple of transient 1302s that
// the agent then recovers from are unaffected.
const RATE_LIMIT_RE = /\[1302\]|\b429\b|rate[\s_-]?limit|too many requests/i;
const RATE_LIMIT_STORM_MIN_HITS = 4;        // this many rate-limit stderr chunks...
const RATE_LIMIT_STORM_WINDOW_MS = 45_000;  // ...sustained at least this long, zero stdout.
                                            // (heartbeat ticks every 30s → trips at the ~60s tick)
...
export function runAgent(agentKey, prompt, { silent = false, timeout: timeoutOverride, onStdout, onStderr, onHeartbeat, inactivityTimeoutMs, isModerator = false } = {}) {
  if (budgetEnabled()) {
    const check = tryConsume(agentKey, { isModerator });
    if (!check.ok) {
      const msg = `[BUDGET_EXCEEDED] reason=${check.reason} key=${agentKey}`;
      if (!silent) process.stderr.write(chalk.red(msg + "\n"));
      return Promise.resolve({ agent: agentKey, response: msg, elapsed: 0, status: "budget_exceeded", tokens: { total: 0, estimated: true } });
    }
  }
  ...
  const agentPromise = new Promise((resolve) => {
    ...
    // Heartbeat + inactivity watchdog (delegate mode only — skipped when
    // onHeartbeat is not provided, so interactive REPL use is unaffected).
    // Fires every 30s: calls onHeartbeat() and optionally kills the child
    // if no stdout/stderr has arrived for inactivityTimeoutMs.
    let lastActivityMs = Date.now();
    // Rate-limit storm tracking (see RATE_LIMIT_* above). Only meaningful in
    // delegate mode, where onHeartbeat drives the check loop below.
    let stdoutEverSeen = false;
    let firstRateLimitMs = null;
    let rateLimitHits = 0;
    if (onHeartbeat) {
      heartbeatId = setInterval(() => {
        if (settledRef.value) return;
        try { onHeartbeat(); } catch {}
        // Rate-limit storm: sustained provider throttling with zero output →
        // the agent is stuck in its retry loop. Bail with a quota signal.
        if (!stdoutEverSeen && firstRateLimitMs
            && rateLimitHits >= RATE_LIMIT_STORM_MIN_HITS
            && (Date.now() - firstRateLimitMs) >= RATE_LIMIT_STORM_WINDOW_MS) {
          settledRef.value = true;
          clearInterval(heartbeatId);
          heartbeatId = null;
          spinner?.stop();
          try { proc.kill("SIGKILL"); } catch {}
          const storminess = Math.round((Date.now() - firstRateLimitMs) / 1000);
          resolve({
            agent: agentKey,
            response: "",
            error: `rate limit reached (1302): provider throttled ${rateLimitHits}x over ${storminess}s with no output — try another agent (e.g. codex)`,
            rateLimited: true,
            elapsed: (Date.now() - startTime) / 1000,
          });
          return;
        }
        if (inactivityTimeoutMs && (Date.now() - lastActivityMs) > inactivityTimeoutMs) {
          settledRef.value = true;
          clearInterval(heartbeatId);
          heartbeatId = null;
          spinner?.stop();
          try { proc.kill("SIGKILL"); } catch {}
          resolve({ agent: agentKey, response: "", error: "inactivity timeout", timedOut: true, timeoutSec: Math.round(inactivityTimeoutMs / 1000), elapsed: (Date.now() - startTime) / 1000 });
        }
      }, 30_000);
    }
    ...
    proc.stdout.on("data", (data) => {
      ...
      stdout += text;
      if (text.trim()) stdoutEverSeen = true; // any real output → disarm storm fast-fail
      if (onStdout) { try { onStdout(text); } catch {} }
      if (!silent) {
        if (firstChunk) {
          spinner?.stop();
    ...
    proc.stderr.on("data", (d) => {
      ...
      const text = d.toString();
      stderr += text;
      // Count rate-limit chunks only while we've seen no real output — that's
      // the retry-trap signature the heartbeat storm check acts on.
      if (!stdoutEverSeen && RATE_LIMIT_RE.test(text)) {
        if (firstRateLimitMs === null) firstRateLimitMs = Date.now();
        rateLimitHits++;
      }
      if (onStderr) { try { onStderr(text); } catch {} }
    });
