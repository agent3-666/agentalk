...
function loadFilteredAgents() {
  ...
  if (_callerIsClaudeCode && !_includeClaude) {
    const skipped = Object.keys(defs).filter(k => defs[k].cmd === "claude");
    for (const k of skipped) delete defs[k];
    if (skipped.length > 0) {
      process.stderr.write(
        chalk.yellow(`⚠ agentalk: [${skipped.join(", ")}] skipped — host is Claude Code (shares quota). Pass --include-claude to override.\n`)
      );
    }
  }
  return defs;
...
export function runAgent(agentKey, prompt, { silent = false, timeout: timeoutOverride, onStdout, onStderr, onHeartbeat, inactivityTimeoutMs } = {}) {
