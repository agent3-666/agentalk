...
function streamOpencode(proc, agent, silent, resolve, spinner) {
...
function startSpinner(agent) {
  ...
  process.stdout.write(`\n${base}${chalk.dim(`${SPINNER[0]} thinking...`)}`);
  const timer = setInterval(() => {
    i = (i + 1) % SPINNER.length;
    process.stdout.write(`\r${base}${chalk.dim(`${SPINNER[i]} thinking...`)}`);
  }, 80);
  return {
    stop() {
      clearInterval(timer);
...
export function runAgent(agentKey, prompt, { silent = false } = {}) {
  ...
  return new Promise((resolve) => {
    ...
    if (agent.output === "ndjson") {
      streamOpencode(proc, agent, silent, resolve, spinner);
      return;
    }
