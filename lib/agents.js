...
export function runAgent(agentKey, prompt, { silent = false, timeout: timeoutOverride } = {}) {
  ...
  const agentPromise = new Promise((resolve) => {
    ...
    proc.on("close", (code) => {
      ...
      resolve({ agent: agentKey, response: stdout.trim(), error: cleanErr, elapsed: parseFloat(elapsed), tokens });
    });
