...
const STRINGS = {
  en: {
    ...
    "agents.add_model_usage":    "Usage: /agents add-model <model-id> [<model-id>...]\nExamples: /agents add-model gpt-4o\n          /agents add-model deepseek/deepseek-chat groq/llama-3.3-70b",
    "agents.add_model_added":    "Added model agent: {key} ({model})",
    "agents.set_key_usage":      "Usage: /agents set-key <provider> <api-key>",
    "agents.set_key_done":       "API key saved for: {provider}",
    "agents.set_key_list":       "Providers with keys: {providers}",
    "agents.set_key_none":       "No API keys configured. Use: /agents set-key <provider> <key>",
    "agents.set_endpoint_usage": "Usage: /agents set-endpoint <provider> <base-url>",
    "agents.set_endpoint_done":  "Custom endpoint set for {provider}: {endpoint}",
...
You are {name} (speaker {pos}/{total}, round {round}). First respond to others' points (agree, question, or add), then judge whether convergence is reached. If the core disagreements are resolved and nothing new remains, add [STOP] at the end. Concise (under 150 words).`,
    ...
    // ── Brainstorm prompts ──
    "prompt.brainstorm_r1": `{history}

You are {name}. Give YOUR unique perspective on the topic. Be original — take an unexpected angle, a contrarian view, or an underexplored dimension. Do NOT restate or echo what others have already said. Under 200 words.`,

    "prompt.brainstorm_rn": `{history}

You are {name}, round {round}. Add a completely NEW angle or dimension not yet raised by anyone. Do NOT repeat, summarize, or reframe what's already been said — only contribute if you have something genuinely new. If you have nothing new to add, end your reply with [STOP]. Under 150 words.`,

    // ── Challenge prompts ──
    "prompt.challenge_r1_first": `{history}

You are {name}, adversarial reviewer. Your job: find flaws. Identify the weakest assumptions, hidden risks, failure modes, and edge cases in the topic. Be specific and constructive — name the exact point of failure. Under 200 words.`,

    "prompt.challenge_r1_mid": `{history}

You are {name} (reviewer {pos}/{total}). Previous reviewers raised some issues. Find the flaws they MISSED — attack different assumptions, expose a different failure mode, or challenge a different premise. Under 200 words.`,

    "prompt.challenge_rn": `{history}

You are {name} (round {round}). Either: (a) poke holes in any rebuttals or defenses offered so far, or (b) raise a new class of risk not yet addressed. If all major risks have been raised and adequately discussed, add [STOP] at the end. Under 150 words.`,

    // ── Deepen prompts ──
    "prompt.deepen_r1_first": `{history}

You are {name}, speaking first. Give an initial analysis of the topic. Identify the single most important underlying question, mechanism, or root cause worth digging into. Under 200 words.`,

    "prompt.deepen_r1_mid": `{history}

You are {name} (position {pos}/{total}). Read what came before. Go ONE LAYER DEEPER on the most interesting thread raised — don't pivot to a new topic, don't repeat what was said, just drill further down into the same direction. Under 200 words.`,

    "prompt.deepen_rn": `{history}

You are {name} (round {round}). Take the deepest insight from the previous speaker and go one level further — ask "why does that happen?", "what's the mechanism behind that?", or "what does that imply?". If maximum useful depth has been reached and further drilling would yield diminishing returns, add [STOP]. Under 150 words.`,

    // ── Command descriptions (autocomplete) ──
    "cmd.debate":    "multi-round serial debate",
    "cmd.panel":     "blind opening round + serial debate",
    "cmd.discuss":   "parallel discussion",
    "cmd.brainstorm": "divergent — maximize unique perspectives",
    "cmd.challenge":  "adversarial — find flaws and risks",
    "cmd.deepen":     "serial depth-first — each agent drills deeper",
    ...
    "cmd.agents_add_model":  "register API model(s) as agent",
    "cmd.agents_set_key":    "save API key for a provider",
...
<one sentence — what this discussion should produce>`,
  ...
  zh: {
    ...
    "agents.add_model_usage":    "用法: /agents add-model <model-id> [<model-id>...]\n示例: /agents add-model gpt-4o\n      /agents add-model deepseek/deepseek-chat groq/llama-3.3-70b",
    "agents.add_model_added":    "已添加模型 agent: {key} ({model})",
    "agents.set_key_usage":      "用法: /agents set-key <provider> <api-key>",
    "agents.set_key_done":       "API key 已保存: {provider}",
    "agents.set_key_list":       "已配置 key 的 provider: {providers}",
    "agents.set_key_none":       "暂无 API key。用 /agents set-key <provider> <key> 配置",
    "agents.set_endpoint_usage": "用法: /agents set-endpoint <provider> <base-url>",
    "agents.set_endpoint_done":  "已为 {provider} 设置自定义 endpoint: {endpoint}",
...
你是 {name}（第 {pos}/{total} 位发言，第 {round} 轮）。请先回应其他人的观点（认同、质疑或补充），再判断是否已收敛。如果你认为核心分歧已解决且无新信息可补充，回复末尾加 [STOP]。简洁（150字以内）。`,
    ...
    // ── Brainstorm prompts (ZH) ──
    "prompt.brainstorm_r1": `{history}

你是 {name}。请就以上话题发表你的**独特**观点。要有原创性——可以是意想不到的角度、反常识的看法，或尚未被探索的维度。不要重复或附和他人已说过的内容。200字以内。`,

    "prompt.brainstorm_rn": `{history}

你是 {name}，第 {round} 轮。请提出一个还没有人提到的**全新角度或维度**。不要重复、总结或改述已有内容——只有你真的有新东西要补充时才发言。如果你确实没有新内容可以贡献，回复末尾加 [STOP]。150字以内。`,

    // ── Challenge prompts (ZH) ──
    "prompt.challenge_r1_first": `{history}

你是 {name}，担任对抗性审查员。你的任务：挑毛病。找出话题中最薄弱的假设、隐藏风险、故障模式和边缘情况。要具体——指出确切的问题点。200字以内。`,

    "prompt.challenge_r1_mid": `{history}

你是 {name}（第 {pos}/{total} 位审查员）。前面的审查员已经指出了一些问题。请找出他们**遗漏的**缺陷——攻击不同的假设，暴露不同的故障模式，或质疑不同的前提。200字以内。`,

    "prompt.challenge_rn": `{history}

你是 {name}（第 {round} 轮）。请选择其一：(a) 对目前提出的任何反驳或辩护继续挑漏洞，或 (b) 提出一类尚未被提及的新风险。如果所有主要风险都已被充分讨论，回复末尾加 [STOP]。150字以内。`,

    // ── Deepen prompts (ZH) ──
    "prompt.deepen_r1_first": `{history}

你是 {name}，第一个发言。对话题进行初步分析，找出最值得深挖的核心问题、机制或根本原因。200字以内。`,

    "prompt.deepen_r1_mid": `{history}

你是 {name}（第 {pos}/{total} 位）。读完前面的分析后，在最有价值的那条线索上**深入一层**——不要转换话题，不要重复已说的，只沿着同一方向继续往深钻。200字以内。`,

    "prompt.deepen_rn": `{history}

你是 {name}（第 {round} 轮）。拿起上一位发言者最深刻的那个洞察，再深入一层——追问"为什么会这样？"、"背后的机制是什么？"、"这意味着什么？"。如果已经挖到足够深且继续下去收益递减，回复末尾加 [STOP]。150字以内。`,

    // ── Command descriptions (autocomplete) ──
    "cmd.debate":              "多轮串行辩论",
    "cmd.panel":               "盲轮开场 + 串行辩论",
    "cmd.discuss":             "并行讨论",
    "cmd.brainstorm":          "发散 — 最大化独特视角",
    "cmd.challenge":           "对抗 — 找漏洞和风险",
    "cmd.deepen":              "接力深化 — 每轮往深钻一层",
    ...
    "cmd.agents_add_model":    "注册 API 模型为 agent",
    "cmd.agents_set_key":      "保存 provider 的 API key",
