...
function printTokenTable(usage, agentKeys, out) {
  ...
  for (const [key, v] of entries) {
    ...
    const label = agent ? agent.label : chalk.bgGray.white(` ${key.slice(0, 7).padEnd(7)} `);
    const est = v.estimated ? "~" : "";
    const totalStr = chalk.dim(`${est}${v.total.toLocaleString()} tokens`);
    const breakdown = v.prompt > 0
      ? chalk.dim(`  ↑${v.prompt.toLocaleString()} ↓${v.completion.toLocaleString()}`)
      : "";
    const turns = chalk.dim(`  · ${v.turns} turn${v.turns !== 1 ? "s" : ""}`);
    out.log(`  ${label}  ${totalStr}${breakdown}${turns}`);
    grandTotal += v.total;
  ...
  if (entries.length > 1) {
    out.log(chalk.dim(`  ${"Total".padEnd(9)}  ${chalk.bold(est + grandTotal.toLocaleString())} tokens`));
  }
  out.log("");
  out.log(chalk.dim(`  ◆ agentalk  ·  ${entries.length} agent${entries.length !== 1 ? "s" : ""}  ·  ${est}${grandTotal.toLocaleString()} tokens`));
  out.log("");
}
...
function isAuthError(error) {
  return /not.?logged.?in|unauthenticated|unauthorized|authentication|sign.?in|log.?in.*required|api.?key|oauth/i.test(error);
}
...
async function judgeConvergence(recentMessages) {
  ...
  const result = await runAgent(getJudgeKey(), t("prompt.judge", { history }), { silent: true, timeout: ORCHESTRATOR_TIMEOUT, isModerator: true });
...
async function summarizeCurrent(context) {
  ...
  const result = await runAgent(getJudgeKey(), t("prompt.summarise", { history }), { silent: true, timeout: ORCHESTRATOR_TIMEOUT, isModerator: true });
  return result.response.trim();
}
...
export async function discuss(topic, context, options = {}) {
  ...
  out.log(header || t("discuss.parallel_header", { rounds: maxRounds }));
  out.log(t("discuss.topic", { topic }));
  out.log(chalk.dim(`  Participants: ${agents.map(k => AGENTS[k]?.name || k).join("  ·  ")}`));
  ...
  for (let round = 1; round <= maxRounds; round++) {
    ...
    for (const r of results) {
      ...
      if (r.response) {
        ...
        } else if (!r.inlineShown) {
          // Only print if not already shown inline by agents.js
          if (isAuthError(r.error)) {
            out.log(chalk.yellow(`⚠ ${name}: not authenticated — run \`${AGENTS[r.agent]?.cmd || r.agent} auth login\` to sign in`));
          } else {
  ...
  for (let turn = 0; turn < maxTurns; turn++) {
    ...
    if (!result.response && result.error) {
      ...
      if (result.timedOut) {
        ...
        out.log(chalk.yellow(t("discuss.busy_skip", { name })));
      } else if (!result.inlineShown) {
        if (isAuthError(result.error)) {
          out.log(chalk.yellow(`⚠ ${name}: not authenticated — run \`${AGENTS[agentKey]?.cmd || agentKey} auth login\` to sign in`));
        } else {
...
// ─── /panel — blind opening round + serial debate ────────────────────
// Phase 1: all agents respond in parallel without seeing each other (blind).
// Phase 2: serial debate rounds, at least 1 full cycle before convergence exit.
export async function panel(topic, context, options = {}) {
  const { maxRounds = 3, agents = Object.keys(AGENTS), capture = false, noJudge = false, noAddUser = false } = options;
  const out = makeOutput(capture);
  const tokenUsage = makeTokenUsage();
  clearStop();
  ...
  const agentNames = agents.map(k => AGENTS[k]?.displayName || k).join(" → ");
  out.log(`\n🎭  Panel · ${agentNames}`);
  out.log(`   Phase 1: independent opening statements  Phase 2: ${maxRounds}-round debate`);
  out.log(t("discuss.topic", { topic }));

  if (!noAddUser) context.add("user", topic);

  // ── Phase 1: Blind round (all respond in parallel, no one sees others yet) ──
  out.log(`\n── Opening Statements ──`);

  const blindResults = await Promise.all(
    agents.map(key => runAgent(key, buildDiscussPrompt(context, key, 1), { silent: capture }))
  );
  ...
  for (const r of blindResults) {
    ...
    if (r.response) {
      ...
      const name = AGENTS[r.agent]?.displayName || r.agent;
      if (r.timedOut) out.log(chalk.yellow(t("discuss.timeout_skip", { name, s: r.timeoutSec })));
      else if (isTransientError(r.error)) out.log(chalk.yellow(t("discuss.busy_skip", { name })));
      else if (!r.inlineShown) {
        if (isAuthError(r.error)) out.log(chalk.yellow(`⚠ ${name}: not authenticated — run \`${AGENTS[r.agent]?.cmd || r.agent} auth login\` to sign in`));
        else out.log(chalk.yellow(t("discuss.error_skip", { name, msg: r.error.split("\n")[0].slice(0, 80) })));
      }
    }
  }
  ...
  // ── Phase 2: Serial debate (at least 1 full cycle before allowing exit) ──
  out.log(`\n── Debate ──`);

  const maxTurns = maxRounds * agents.length;
  const stoppedAgents = new Set();
  let skippedInRow = 0;
  ...
  for (let turn = 0; turn < maxTurns; turn++) {
    ...
    const agentKey = agents[turn % agents.length];
    const position = (turn % agents.length) + 1;
    const roundNum = Math.floor(turn / agents.length) + 1; // 1-indexed serial rounds

    if (turn % agents.length === 0) out.log(t("discuss.round", { n: roundNum }));

    // Pass roundNum+1 so prompts know this is not a cold start (blind round was round 1)
    const prompt = buildDebatePrompt(context, agentKey, position, agents.length, roundNum + 1, "debate");
    const result = await runAgent(agentKey, prompt, { silent: capture });
    ...
    if (!result.response && result.error) {
      skippedInRow++;
      const name = AGENTS[agentKey]?.displayName || agentKey;
      if (result.timedOut) out.log(chalk.yellow(t("discuss.timeout_skip", { name, s: result.timeoutSec })));
      else if (isTransientError(result.error)) out.log(chalk.yellow(t("discuss.busy_skip", { name })));
      else if (!result.inlineShown) {
        if (isAuthError(result.error)) out.log(chalk.yellow(`⚠ ${name}: not authenticated — run \`${AGENTS[agentKey]?.cmd || agentKey} auth login\` to sign in`));
        else out.log(chalk.yellow(t("discuss.error_skip", { name, msg: result.error.split("\n")[0].slice(0, 80) })));
      }
      if (skippedInRow >= agents.length) { out.log(chalk.red(t("discuss.all_failed"))); break; }
      continue;
    }
    skippedInRow = 0;
    ...
    if (result.response) {
      ...
      // Honor [STOP] from serial round 1 onwards (blind round already completed)
    ...
    // Convergence check at end of each serial cycle (first check = after 1 full debate round)
    const isEndOfCycle = (turn + 1) % agents.length === 0;
    if (!noJudge && isEndOfCycle) {
      if (stopSignal.requested) { await gracefulStop(context, out); return finalize(); }
      out.log(t("discuss.judge_checking"));
      const verdict = await judgeConvergence(context.messages.slice(-agents.length * 2));
      showJudgeVerdict(verdict, out);
...
// ─── /brainstorm — divergent parallel rounds ─────────────────────────
export function brainstorm(topic, context, options = {}) {
  const agentNames = Object.values(AGENTS).map(a => a.displayName).join(" · ");
  return discuss(topic, context, {
    maxRounds: 4,
    ...options,
    promptType: "brainstorm",
    header: `\n🧠  Brainstorm · ${agentNames}`,
  });
}

// ─── /challenge — adversarial serial review ──────────────────────────
export function challenge(topic, context, options = {}) {
  const agentNames = Object.keys(AGENTS).map(k => AGENTS[k]?.displayName || k).join(" → ");
  return debate(topic, context, {
    maxTurns: 12,
    ...options,
    promptType: "challenge",
    header: `\n⚔️  Challenge · ${agentNames}`,
  });
}

// ─── /deepen — serial depth-first drilling ──────────────────────────
export function deepen(topic, context, options = {}) {
  const agentNames = Object.keys(AGENTS).map(k => AGENTS[k]?.displayName || k).join(" → ");
  return debate(topic, context, {
    maxTurns: 12,
    ...options,
...
export async function moderatedSession(request, context, options = {}) {
  ...
  const planResult = await runAgent(judgeKey, t("prompt.mod_plan", { request, history, agents: agentList, briefing_hint: briefingHint, url_hint: urlHint }), { silent: true, timeout: ORCHESTRATOR_TIMEOUT, isModerator: true });
...
export async function broadcast(message, context, targets) {
  ...
  const out = makeOutput(false);
  printTokenTable(tokenUsage, targets, out);
