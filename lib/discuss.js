...
export async function moderatedSession(request, context, options = {}) {
  ...
  // Planning uses a short timeout (max 60s) — it only needs to output ~5 lines
  const planningTimeout = Math.min(60, getGlobalTimeout());
  const planResult = await runAgent(judgeKey, t("prompt.mod_plan", { request, history, agents: agentList }), { silent: true, timeout: planningTimeout });
