...
// Resolves immediately when stop is requested — used to race against runAgent()
function waitForStop(agentKey) {
  return new Promise(resolve => {
    if (stopSignal.requested) { resolve({ agent: agentKey, response: "", stopped: true }); return; }
    const t = setInterval(() => {
      if (stopSignal.requested) {
        clearInterval(t);
        resolve({ agent: agentKey, response: "", stopped: true });
      }
    }, 50);
  });
}
...
export async function discuss(topic, context, options = {}) {
  ...
  for (let round = 1; round <= maxRounds; round++) {
    ...
    out.log(t("discuss.round", { n: round }));

    const results = await Promise.all(
      agents.map((key) => Promise.race([
        runAgent(key, buildDiscussPrompt(context, key, round), { silent: capture }),
        waitForStop(key),
      ]))
    );
...
export async function debate(topic, context, options = {}) {
  ...
  for (let turn = 0; turn < maxTurns; turn++) {
    ...
    const result = await Promise.race([
      runAgent(agentKey, prompt, { silent: capture }),
      waitForStop(agentKey),
    ]);

    if (result.stopped) {
      // Stop signal fired mid-turn — exit at top of next iteration
      continue;
    }
...
export async function moderatedSession(request, context, options = {}) {
  ...
  if (judgeAgent && !capture) {
    out.log(`\n${judgeAgent.label}${chalk.dim(" [Moderator]")}`);
    out.log(`${judgeAgent.color("│")} ${chalk.dim(t("mod.analyzing"))}`);
    out.log(`${judgeAgent.color("╰")}`);
  }

  const planResult = await runAgent(judgeKey, t("prompt.mod_plan", { request, history, agents: agentList }), { silent: true });
