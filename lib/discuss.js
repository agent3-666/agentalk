  ...
  try {
    ...
    for (const m of context.messages) {
      if (m.role === "user") {
        ...
        // Language-neutral internal markers for conclusions
        if (/^\[(CONCLUSION|DEBATE_CONCLUSION|TIMEOUT|ABORTED)\]/.test(m.content)) {
          lines.push(`---\n\n${t("summary.md_conclusion")}\n\n${m.content.replace(/^\[.*?\]\s*/, "")}\n`);
        }
...
// ─── Judge / Moderator ───────────────────────────────────────────────
// The moderator (judge) is always Claude. It runs silently but its verdict
// is displayed with its own label so users can see Claude is moderating.
const JUDGE_KEY = "claude";

function getJudgeAgent() {
  return AGENTS[JUDGE_KEY] || null;
}
...
async function judgeConvergence(recentMessages) {
  ...
  const result = await runAgent(JUDGE_KEY, t("prompt.judge", { history }), { silent: true });
...
async function summarizeCurrent(context) {
  ...
  const result = await runAgent(JUDGE_KEY, t("prompt.summarise", { history }), { silent: true });
