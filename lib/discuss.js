...
async function judgeConvergence(recentMessages) {
  ...
  const result = await runAgent("claude", t("prompt.judge", { history }), { silent: true });
...
async function summarizeCurrent(context) {
  console.log(chalk.dim(t("discuss.summarising")));
  const history = formatHistory(context.messages.slice(-12));
  const result = await runAgent("claude", t("prompt.summarise", { history }), { silent: true });
      ...
      if (roundNum >= 2 && hasStopSignal(result.response)) {
        ...
        if (stopped < total) {
          ...
          if (verdict.converged) {
            ...
            out.log(t("discuss.judge_no", { reason: verdict.reason }));
            stoppedAgents.clear();
          }
        }
      }
