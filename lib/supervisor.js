...
const DEFAULT_CAPABILITIES = {
  ...
  codex: {
    ...
    cost_tier: "mid",
    good_for: ["quick code edits", "one-shot scripts", "CLI-heavy tasks"],
  ...
  gemini: {
    ...
    cost_tier: "mid",
    good_for: ["reading large docs", "PDF analysis", "whole-codebase scans"],
  ...
  opencode: {
    ...
    context_window: 128000,
    cost_tier: "low",
    good_for: ["translation", "simple tasks", "cost-sensitive work"],
...
export function readCapabilities() {
  ...
  if (!existsSync(CAPABILITY_PATH)) {
    atomicWriteJson(CAPABILITY_PATH, DEFAULT_CAPABILITIES);
    return { ...DEFAULT_CAPABILITIES };
  }
  try {
    const loaded = JSON.parse(readFileSync(CAPABILITY_PATH, "utf-8"));
    // Ensure every active agent has at least a stub entry
    for (const key of Object.keys(AGENTS)) {
      if (!loaded[key]) loaded[key] = DEFAULT_CAPABILITIES[key] || {
        strengths: [], good_for: [], cost_tier: "unknown",
      };
    }
    return loaded;
  } catch {
