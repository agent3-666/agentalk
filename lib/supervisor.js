...
import { resolveSessionByScan } from "./session-discovery.js";
...
export function parseQuotaSignal(result) {
  ...
  // A non-empty answer means the agent RECOVERED from anything it logged to
  // stderr (batch agents like zcode retry past transient 1302/rate-limit
  // errors, then succeed — but the noisy stderr lingers). Treat that as ok so
  // a successful-but-noisy run isn't mislabeled quota_exceeded/auth_failed.
  // Genuine terminal failures produce no response and still fall through below.
...
const DEFAULT_CAPABILITIES = {
  ...
  zcode: {
    billing: "subscription",
    priority: "preferred",
    note: "ZCode running GLM-5.2 (z.ai Coding Plan) — cheap subscription. Best for BOUNDED coding sub-tasks (a few files, bug fix, tests, chinese text). Do NOT send heavy multi-file / long agentic reviews: bursts trip z.ai's 1302 rate-limit whose retry loop otherwise burns the whole timeout with zero output — route those to codex. agentalk now detects that 1302 storm and fast-fails in ~60s with a quota_exceeded signal (instead of a 23-min silent hang), so a rate-limited zcode returns quickly and you should retry on codex. Do NOT use for non-coding work (strategy / prose brainstorming): z.ai actively bans non-coding use of the Coding Plan. GLM-5.2 also costs 3x quota at peak 14:00-18:00 UTC+8.",
    strengths: ["code", "chinese", "low cost", "GLM-5.2"],
    context_window: 200000,
    cost_tier: "low",
    good_for: ["bounded code edits", "reviewing a few files", "tests", "chinese text work", "cost-sensitive coding"],
    weakness: ["heavy multi-file agentic tasks (1302 rate-limit → use codex)", "non-coding use (account ban risk)"],
  },
  opencode: {
    billing: "subscription",
    priority: "normal",
    note: "OpenCode running the same GLM Coding Plan as zcode (shared quota pool, different harness). Disabled by default in favor of zcode; enable with `enable opencode` if you need a second GLM harness.",
    ...
    weakness: ["shares zcode's GLM quota (same 1302 rate-limit)", "model depends on backend config"],
  },
};
...
// Resolve a session reference, trying the explicit registry first and
// then falling back to scanning ~/.claude/projects/ for a basename
// match. This is the path @<name> uses — explicit naming is an
// ergonomic shortcut, not a precondition.
//
// Returns:
//   { source: "alias",   match: { name, cwd, session_id, agent } }
//   { source: "scan",    match: { name, cwd, session_id, agent } }   // name=basename
//   { ambiguous: true,   candidates: [{ basename, cwd, last_active, latest_session }, ...] }
//   null
export function resolveSession(query) {
  if (!query) return null;
  const sessions = readSessions();
  if (sessions[query]) {
    return { source: "alias", match: { name: query, ...sessions[query] } };
  }
  const scan = resolveSessionByScan(query);
  if (!scan) return null;
  if (scan.ambiguous) return scan;
  const p = scan.match;
  return {
    source: "scan",
    match: {
      name: p.basename,
      cwd: p.cwd,
      session_id: null, // let claude resume latest via -c
      agent: "claude",
      last_active: p.last_active,
      latest_session: p.latest_session,
    },
  };
}
...
// ─── Inbox "seen" cursor ───────────────────────────────────────────
// inbox.jsonl is append-only, so we track what a session has already
// been shown via a separate cursor file holding the ISO `ts` of the
// last surfaced record. readUnseenInbox returns everything newer.
// Used by the `inbox-hook` command so a running Claude Code session is
// told (at its next turn / on startup) about delegations it received.
function inboxCursorPath(cwd) {
  return join(cwd, ".agentalk", "inbox.seen");
}

export function readInboxCursor(cwd) {
  const p = inboxCursorPath(cwd);
  if (!existsSync(p)) return null;
  try { return readFileSync(p, "utf-8").trim() || null; } catch { return null; }
}

export function writeInboxCursor(cwd, ts) {
  try {
    ensureProjectDir(cwd);
    writeFileSync(inboxCursorPath(cwd), String(ts));
    return true;
  } catch {
    return false;
  }
...
export function readUnseenInbox(cwd, { limit = 50 } = {}) {
  const cursor = readInboxCursor(cwd);
  const all = readInbox(cwd, { limit: 500 });
  const unseen = cursor ? all.filter(r => r.ts && r.ts > cursor) : all;
  return unseen.slice(-limit);
}
...
// ─── Pre-flight probe: cheap liveness check before real delegation ──
// Sends a single "hi" with a short timeout, bypassing the delegation
// envelope entirely. Failures here mean the agent CLI is broken
// (auth/quota/unreachable) — we'd rather waste ~2-3k probe tokens than
// burn a full delegation on a dead agent. Persists the result to
// quota.json so `doctor` stays current.
const PROBE_TIMEOUT_SEC = 60;
export async function probeAgent(agent) {
  const result = await runAgent(agent, "hi", { silent: true, timeout: PROBE_TIMEOUT_SEC });
...
// Default inactivity (no-stdout/no-stderr) watchdog. The watchdog kills
// the child if no stream chunk arrives for this many seconds. Resolution
// order at delegate time:
//   1. CLI flag (--inactivity-sec N)        — caller override
//   2. AGENTS[key].inactivity_timeout_sec   — per-agent default (e.g. opencode = 900)
//   3. DEFAULT_INACTIVITY_SEC               — this constant
// 0 (anywhere in the chain) disables the watchdog; only total --timeout
// bounds the call. Raised from the old hardcoded 300s: large refactor
// tasks routinely have 5-10 min silent stretches inside a single tool call.
const DEFAULT_INACTIVITY_SEC = 600;
...
async function delegateCore({
  ...
  taskId = null,
  mainAgent = "main",
  timeout = null,               // total timeout in seconds; null → DEFAULT_DELEGATE_TIMEOUT_SEC
  inactivitySec = null,         // stream-silence watchdog in seconds; 0 = disable.
                                // null → AGENTS[key].inactivity_timeout_sec ?? DEFAULT_INACTIVITY_SEC
  resumeStep = null,            // { task_id, step_id } — prepend prior stdout to brief
}) {

  const capabilities = readCapabilities();

  // Pre-flight probe: verify the agent is reachable + authenticated before
  // burning the real task's tokens. ~2-3k tokens vs 50k+ for a real call —
  // worth the overhead. Skip task bookkeeping entirely on probe failure.
  const probeSignal = await probeAgent(agent);
  if (probeSignal.outcome !== "ok") {
    const freshQuotas = readQuotaState();
    return {
      status: probeSignal.outcome,
      agent,
      task_id: null,
      step_id: null,
      findings: null,
      ...
      diagnostics: {
        ...
        detail: `pre-flight probe failed: ${probeSignal.detail || "no detail"}`,
        reset_hint: probeSignal.reset_hint,
        suggestion: suggestAlternatives(agent, freshQuotas, capabilities),
        elapsed_ms: 0,
        tokens: null,
  ...
  // Resolve inactivity watchdog: explicit caller value > per-agent default > global default.
  // 0 disables the watchdog entirely (only total --timeout bounds the call).
  const agentDef = AGENTS[agent];
  const resolvedInactivitySec = inactivitySec ?? agentDef?.inactivity_timeout_sec ?? DEFAULT_INACTIVITY_SEC;
  const inactivityTimeoutMs = resolvedInactivitySec > 0 ? resolvedInactivitySec * 1000 : null;
  ...
  const result = await runAgent(agent, prompt, {
    ...
    onHeartbeat: () => emit(task.id, { type: "heartbeat", step_id: step.id, agent }),
    inactivityTimeoutMs,
  });
...
export async function delegateSession(params) {
  ...
  const resolved = resolveSession(name);
  if (!resolved) {
    throw new Error(`No session matches "${name}". Run \`agentalk-delegate sessions\` to list discoverable ones, or register an alias via \`agentalk-delegate register-session ${name} --cwd <path>\`.`);
  }
  if (resolved.ambiguous) {
    const lines = resolved.candidates.slice(0, 8).map(c => `  - ${c.basename}  (${c.cwd})`).join("\n");
    throw new Error(`"${name}" is ambiguous — ${resolved.candidates.length} sessions match:\n${lines}\n\nDisambiguate by passing a full cwd path, a longer basename prefix, or register an alias.`);
  }
  const sess = resolved.match;
  ...
  try {
    return await delegateSessionCore({
      ...params,
    ...
    return systemErrorResult(`@${sess.name}`, err);
  }
}

async function delegateSessionCore({ sessionName, session, brief, timeout = null, taskId = null, mainAgent = "main", resolveSource = "alias" }) {
  const DEFAULT_DELEGATE_TIMEOUT_SEC = 600;
  const effectiveTimeout = timeout ?? DEFAULT_DELEGATE_TIMEOUT_SEC;
